...
function Capsule({
  ...
  return (
    <div
      className={`relative w-full max-w-xs sm:max-w-sm mx-auto border-4 sm:border-5 border-gray-400 bg-black/90 rounded-full shadow-lg flex flex-col items-center pt-8 pb-5 gap-3 ${
        !isOpen ? "select-none cursor-not-allowed" : ""
      }`}
    >
      <div className="flex gap-2 items-center text-white text-base sm:text-xl font-bold">
        Type: <span>{typeIcons[type]}</span>
        <span>{type?.charAt(0).toUpperCase() + type?.slice(1)}</span>
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap justify-center px-4">
        ...
        <LockKeyhole size={18} className="text-orange-400" />
        <span className="font-semibold text-orange-400 text-xs sm:text-sm text-center">
          {new Date(created_At).toLocaleDateString("en-US", {
            month: "short",
            day: "2-digit",
            year: "numeric",
      ...
      <div className="flex items-center gap-2 flex-wrap justify-center px-4">
        <h2 className="font-bold text-white text-sm sm:text-base">Open On:</h2>
        <CalendarClock size={18} className="text-green-500" />
        <span className="font-semibold text-green-500 text-xs sm:text-sm text-center">
          {new Date(openDate).toLocaleDateString("en-US", {
            month: "short",
            day: "2-digit",
            year: "numeric",
      ...
      {/* Countdown */}
      <div className="text-green-500 text-xl sm:text-2xl font-bold text-center">
        {!isOpen && timeLeft ? (
          <div className="text-red-500">
            {timeLeft.days}
            <span className="text-base sm:text-lg text-white">d</span>{" "}
            {timeLeft.hours}
            <span className="text-base sm:text-lg text-white">h</span>{" "}
            {timeLeft.minutes}
            <span className="text-base sm:text-lg text-white">m</span>{" "}
            {timeLeft.seconds}
            <span className="text-base sm:text-lg text-white">s</span>
          </div>
        ) : (
          <div className="text-green-500">
            00<span className="text-base sm:text-lg text-white">d</span> 00
            <span className="text-base sm:text-lg text-white">h</span> 00
            <span className="text-base sm:text-lg text-white">m</span> 00
            <span className="text-base sm:text-lg text-white">s</span>
          </div>
        )}
      </div>
      ...
      <div>
        <Button
          className={`w-28 sm:w-32 flex items-center justify-center gap-2 text-sm sm:text-base ${
            isOpen
              ? "bg-green-600 hover:bg-green-700"
              : "bg-red-500 cursor-not-allowed hover:bg-red-500"
          }`}
